...
% x_{1,2} = +/- 1.9. The resultings bumps are further apart, i.e. we observe bump
% repulsion.
%
% See Fig. 15 in 'A dynamic neural field model of continuous input integration'
...
T = 200; dt = 0.01; tDim = 0:dt:T; M = numel(tDim);
...
%% inputs
crit_dist = 1.9;  % distance of the input centers from x=0
A_I = 1.5; sigma_I = 1.0;
Input = zeros(M, N);
I_S1 = A_I * gauss(xDim, crit_dist, sigma_I);
I_S2 = A_I * gauss(xDim, -crit_dist, sigma_I);
Input(1/dt:2/dt, :) = repmat((I_S1 + I_S2),1+(1/dt),1);
...
figure
plot(xDim,I_S1+I_S2,'Color',[1 1 1].*0.7,'linewidth',4), hold on
